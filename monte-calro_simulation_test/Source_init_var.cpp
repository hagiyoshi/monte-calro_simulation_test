...
const double radius_nucleus_t2 = 6.0;
...
double S_matrix(const double d_square)
{
	return exp(-init_c*d_square);
}


double k_0_4_fuc(const double p_mom, const double r_dis, const double b_imp, const double cos_theta)
{
	double rtn_function = p_mom*exp(-r_dis*r_dis / Gauss_param / Gauss_param)*bessel_ink_0_4(p_mom*r_dis)*(1.0 - S_matrix(dist2_rad_imp(r_dis, b_imp, cos(cos_theta))));
...
double calculate_k_0_4_ncvmp_sp_conf(const double b_1, const double by_1, const double b_2, const double by_2,
	const double b_3, const double by_3, const double b_4, const double by_4,
	double& temp2_0_4, double& temp_0_2, double& temp2_0_2)
{
	double error_temp = 0;
...
{
	return p_mom*exp(-r_dis*r_dis / Gauss_param / Gauss_param)*bessel_ink_2_4(p_mom*r_dis)*S_matrix(dist2_rad_imp(r_dis, b_imp, cos(cos_theta)))*cos(2.0*cos_theta);
}
...
double calculate_k_2_4_ncvmp_sp_conf(const double b_1, const double by_1, const double b_2, const double by_2,
	const double b_3, const double by_3, const double b_4, const double by_4,
	double& temp2_2_4, double& temp_2_2, double& temp2_2_2)
{
	double error_temp = 0;
...
int print(){
...
#ifdef CONFINE
	ofilename << "c_2_4_confine_maxP_"<< Upper_momk << "_Nphi_" << NPHI << "_N_" << N << ".txt";
#else
	ofilename << "c_2_4_normal_maxP_" << Upper_momk << "_Nphi_" << NPHI << "_N_" << N << ".txt";
#endif
	...
	ofs_res  << "#init_c \t c_2_4 \t error \t s_2_4 \t error \t c_2_2 \t error\t v_2_2 \t error \n";
	...
	for (int i = 1; i  <= END_T / OUTPUT_DELTA_T + 1e-6; ++i){
		ofs_res << i*OUTPUT_DELTA_T << "\t" << c_2_4[i] <<"\t"<<error_c_2_4[i] << "\t" << s_2_4[i] << "\t" << error_s_2_4[i] << "\t" << c_2_2[i] << "\t" << error_c_2_2[i]
			<< "\t" << sqrt(c_2_2[i]) << "\t" << error_c_2_2[i]/sqrt(2.0)/ sqrt(c_2_2[i]) << "\n";
		}
	return 0;
}
...
int main()
{
	...
	ofilename3 << "sum_k_2_4_k_spline_initc_1_Nphi_" << NPHI << "_N_" << N << ".txt";
	ofstream ofs_res3(ofilename3.str().c_str());



	for (int int_tau = 1; int_tau <= END_T / OUTPUT_DELTA_T+1e-6;++int_tau){
		init_c = int_tau*OUTPUT_DELTA_T;
...
#ifdef CONFINE
std::normal_distribution<> dist_1(0.0, 6.0 / Lamda_QCD ), dist_2(0.0, 6.0 / Lamda_QCD), dist_3(0.0, 6.0 / Lamda_QCD),
dist_4(0.0, 6.0 / Lamda_QCD),
dist_p1(0.0, 6.0 / Lamda_QCD), dist_p2(0.0, 6.0 / Lamda_QCD), dist_p3(0.0, 6.0 / Lamda_QCD),
dist_p4(0.0, 6.0 / Lamda_QCD);
#else
...
for (int i = 0; i < N; ++i)
{
	...
	double temp_0_4 = calculate_k_0_4_ncvmp_sp_conf(dist_1(engine_1), dist_p1(engine_p1), dist_2(engine_2), dist_p2(engine_p2),
		dist_3(engine_3), dist_p3(engine_p3), dist_4(engine_4), dist_p4(engine_p4), temp2_0_4, temp_0_2, temp2_0_2);
	double temp_2_4 = calculate_k_2_4_ncvmp_sp_conf(dist_1(engine_1), dist_p1(engine_p1), dist_2(engine_2), dist_p2(engine_p2),
