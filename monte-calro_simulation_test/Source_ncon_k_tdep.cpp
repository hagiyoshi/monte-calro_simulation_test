...
//#define separate_Gaussian

#define uniformally_random
...
const double limit_mom = 6.0;
const int num_mom = 10;
...
*/
const double Lamda_QCD_nucleus = Lamda_QCD /6.0;

double Limit_of_impact_param = 10.0*Lamda_QCD;
...
double calculate_k_0_4_radisalcvmp(const double b_1, const double bp_1, const double b_2, const double bp_2,
	const double b_3, const double bp_3, const double b_4, const double bp_4, const double Upper_momk,
...
void calculate_k_n_4_ncvmp_sp(const double b_1, const double by_1, const double b_2, const double by_2,
	const double b_3, const double by_3, const double b_4, const double by_4, const double Upper_momk,
...
int print(const double tau, const double Upper_momk, const int epoch){

	ostringstream ofilename;
#ifdef uniformally_random
		ofilename << "c_2_4_k_spline_initc_" << init_c << "_upperlimb_" << Limit_of_impact_param << "_Nphi_" << NPHI << "_N_" << N << "_epoch_" << epoch << "_tau_" << tau << ".txt";
#else
		ofilename << "c_2_4_k_spline_initc_" << init_c << "_Nphi_" << NPHI << "_N_" << N << "_epoch_" << epoch << ".txt";
#endif
	ofstream ofs_res(ofilename.str().c_str());

	ofs_res  << "#mom \t c_2_4 \t error \t s_2_4 \t error \t c_2_2 \t error \n";

	for (int i = 1; i <= num_mom + 1e-6; ++i){
		ofs_res << i*limit_mom / num_mom << "\t" << c_2_4[i] <<"\t"<<error_c_2_4[i] << "\t" << s_2_4[i] << "\t" << error_s_2_4[i] << "\t" << c_2_2[i] << "\t" << error_c_2_2[i] << "\n";
		}
	return 0;
}


int print_impact(const double tau, const double Upper_momk, const int epoch) {
	ostringstream ofilename_i;
	ofilename_i << "K_2_4_initc_1_tau_" << tau << "_Nphi_" << NPHI << "_epoch_" << epoch << ".txt";
...
int main()
{
	...
	ofilename3 << "sum_k_2_4_k_spline_initc_" << init_c << "_Nphi_" << NPHI << "_N_" << N << "_epoch_" << epoch << ".txt";
	...
	double Upper_momk = 0;
	double  tau = 2.0;
	for (int int_mom = 1; int_mom <= num_mom + 1e-6; ++int_mom) {
		Upper_momk = int_mom*limit_mom / num_mom;
		initial(tau);
...
#pragma omp parallel for num_threads(4)
		...
		print_impact(tau,Upper_momk,epoch);
...
#else
#ifdef uniformally_random
			std::uniform_real_distribution<> dist_1(-Limit_of_impact_param, Limit_of_impact_param), dist_2(-Limit_of_impact_param, Limit_of_impact_param), dist_3(-Limit_of_impact_param, Limit_of_impact_param),
				dist_4(-Limit_of_impact_param, Limit_of_impact_param),
				dist_p1(-Limit_of_impact_param, Limit_of_impact_param), dist_p2(-Limit_of_impact_param, Limit_of_impact_param), dist_p3(-Limit_of_impact_param, Limit_of_impact_param),
				dist_p4(-Limit_of_impact_param, Limit_of_impact_param);
#else
		//not separete and uniformally
...
#pragma omp parallel for reduction(+:sum_k_0_4,sum_k_2_4,sum2_k_0_4,sum2_k_2_4,sum_k_0_2,sum_k_2_2,sum2_k_0_2,sum2_k_2_2) num_threads(4)
		for (int i = 0; i < N; ++i)
		{
			...
			calculate_k_n_4_ncvmp_sp(dist_1(engine_1), dist_p1(engine_p1), dist_2(engine_2), dist_p2(engine_p2),
				dist_3(engine_3), dist_p3(engine_p3), dist_4(engine_4), dist_p4(engine_p4), Upper_momk, temp_0_4, temp2_0_4, temp_0_2, temp2_0_2
		...
		c_2_4[int_mom] = sum_k_2_4 / sum_k_0_4 - 2.0*(sum_k_2_2 / sum_k_0_2)*(sum_k_2_2 / sum_k_0_2);
		...
		c_2_2[int_mom] = c_2_4_2;
		error_c_2_2[int_mom] = error_2;
		s_2_4[int_mom] = c_2_4_4;
		error_s_2_4[int_mom] = error_4;
		error_c_2_4[int_mom] = sqrt(error_4*error_4 + 4.0*2.0*error_2*error_2*c_2_4_2*c_2_4_2);


		ofs_res3 << Upper_momk << " \t " << sum_k_0_4 << " \t " << sum2_k_0_4 << " \t " << sum_k_2_4 << " \t " << sum2_k_2_4
			<< " \t " << sum_k_0_2 << " \t " << sum2_k_0_2 << " \t " << sum_k_2_2 << " \t " << sum2_k_2_2 << "\n";


	}

	print(tau,Upper_momk, epoch);
}
