...
double Limit_of_impact_param = 12.0*Lamda_QCD;
...
int print(const int epoch){

	ostringstream ofilename;
#ifdef uniformally_random
		ofilename << "c_2_4_k_spline_initc_" << init_c << "_upperlimb_" << Limit_of_impact_param << "_Nphi_" << NPHI << "_N_" << N << "_epoch_" << epoch << ".txt";
#else
...
int print_impact(const double tau, const int epoch) {
...
#pragma omp parallel for num_threads(4)
		...
		print_impact(int_tau*OUTPUT_DELTA_T,epoch);
