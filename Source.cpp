...
vector<double> c_2_4(100, 0);
vector<double> error_c_2_4(100, 0);
...
double dist2_rad_imp(const double relat, const double impact_param, const double cos_theta){
	return Radius_Nuc*Radius_Nuc*relat*relat
...
{
	return p_mom*exp(-r_dis*r_dis/Gauss_param/Gauss_param)*bessel_ink_0_4(p_mom*r_dis)*sol_BK_sp(gain_x_val(dist2_rad_imp(r_dis, b_imp, cos(cos_theta))));
}
...
double k_0_4_factfunc(const double p_mom, const double b_imp, double *error)
{
	...
	for (int phi = 0; phi <= NPHI; ++phi)
	{
		...
		if (isinf(i) || isnan(i)) { cout << "k_0_4_isinf isnan b_imp " << b_imp << "\t phi" << phi*h_phi_intmeasure << "\t error" << err << "\n"; }
...
double k_2_4_factfunc(const double p_mom, const double b_imp, double *error)
{
	...
	for (int phi = 0; phi <= NPHI; ++phi)
	{
		...
		if (isinf(i) || isnan(i)) { cout << "k_2_4_isinf isnan b_imp " << b_imp << "\t phi" << phi*h_phi_intmeasure << "\t error" << err << "\n"; }
...
#ifdef OMP
#pragma omp parallel for reduction(+:test_integration,test2_integration,test_integration_2)
#endif
